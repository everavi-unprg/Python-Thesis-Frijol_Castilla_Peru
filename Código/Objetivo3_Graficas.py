...
for dept in ['LAMBAYEQUE', 'PIURA']:
    # CV Periodo Oficial (2012-2024) - Usando ddof=0 para desviación poblacional
    data = export_anuales[export_anuales['DEPARTAMENTO_DESCRIPCION'] == dept]['FOBUSD']
    cv = (data.std(ddof=0) / data.mean()) * 100
    cv_data.append({'Región': dept, 'CV': cv})

    # CV Análisis de Sensibilidad (2013-2024) - Usando ddof=0 para desviación poblacional
    data_sens = export_anuales[(export_anuales['DEPARTAMENTO_DESCRIPCION'] == dept) & (export_anuales['AÑO_DECLARACION'] >= 2013)]['FOBUSD']
    cv_sens = (data_sens.std(ddof=0) / data_sens.mean()) * 100
...
fig1.savefig(os.path.join(graf_carpeta, "Objetivo3_IHH_Final_Norma2023.svg"),dpi=300)
plt.close(fig1)
...
fig2.savefig(os.path.join(graf_carpeta,"Objetivo3_CR4_Final.svg"), dpi=300)
plt.close(fig2)
...
fig3.savefig(os.path.join(graf_carpeta, "Objetivo3_CV_Final.svg"),dpi=300)
plt.close(fig3)
